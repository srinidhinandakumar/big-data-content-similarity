...
def createCluster(inputCSV,argNum):
    ...
    for i in range(len(row)):
        if "x-coordinate" in row[i][0].split(","):
            ...
            data[str(round(float(column[argNum]), 2))]=[]         #Cluster based on the argument number passed
    ...
    for i in range(len(row)):
        if "x-coordinate" in row[i][0].split(","):
            ...
            column = row[i][0].split(",")
            second={}
            second["name"]=column[0]+"  "+column[1]
            second["size"]=column[2]
            data[str(round(float(column[argNum]), 2))].append(second)          #Cluster based on the argument number passed
            
    clusterList = []
    i=0
    for elem in data.keys():
        first={}
        first["name"] = str(elem) + "  cluster " + str(i)
